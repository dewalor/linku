defmodule Linku.Repo.Migrations.AddKeyAndNameToInvitations do
  ...
  def up do
    ...
    execute "update invitations set invitee_name = LOWER(SUBSTRING(MD5(''||NOW()::TEXT||RANDOM()::TEXT) FOR 12)) where key is NULL;"

    execute "CREATE UNIQUE INDEX ON invitations (lower(key));"

    execute "ALTER TABLE invitations ALTER COLUMN key SET NOT NULL;"
    execute "ALTER TABLE invitations ALTER COLUMN invitee_name SET NOT NULL;"
  end
